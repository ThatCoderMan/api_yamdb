...
class TestDockerfileCompose:
    def test_infra_structure(self):
        assert 'infra' in os.listdir(root_dir), f'Проверьте, что в пути {root_dir} указана папка `infra`'
        assert os.path.isdir(infra_dir_path), f'Проверьте, что {infra_dir_path} - это папка, а не файл'
    ...
    def test_docker_compose_file(self):
        ...
        assert re.search(
            r'image:\s+([a-zA-Z0-9]+)\/([a-zA-Z0-9_\.])+(\:[a-zA-Z0-9_-]+)?', docker_compose
